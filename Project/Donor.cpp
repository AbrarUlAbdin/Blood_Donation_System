...
void Donor::functionality()
{ 	
	//virtual function for options
	std::cout<<std::endl;
	...
	std::cin>>n;
	std::cout<<std::endl;
	} while (n>4 || n<1); //loop for validation if user inpt input a wrong number than i will ask again to input

	Sleep(800);
	//conditions for required thing like log in, sign up etc
	if (n==1) 
	{
		logIn (); //calling function for log in
	}
	else if (n==2)
	{
		signUp (); //calling function to sign up
	}
	else if (n==3)
	{
		programInfo(); //calling function for program info
	}
	else if (n==4)
	{
		exit(); //calling function for exit
	}
}
void Donor::programInfo() { //defination for programInfo function which contains info about program and developers
	...
	std::cout<<std::endl;
	functionality(); //after printing programInfo again calling options function to show options again to do further things
}
void Donor::exit(){ //defination of exit function which will print a function and exit whole program

		std::cout<<" Thank You For Your Time, Have a Good Day."<<std::endl;
		std::cout<<std::endl;

}
void Donor::signUp() { //defination of sign up function which will help the user to sign up and this function will take whole info of user and will save it to a file
	...
	if(age<17) { //condition for age restriction so that below 17 could not sign up
		...
		userName += "Donor.txt"; //after successful sign up adding .txt extension to user name so a .txt which username will be created and whole user data will be stored there 
...
void Donor::logIn() { //defining log in function
	std::cout<<std::endl;
	std::cout<<std::endl;
	std::cout<<"Username : ";
	std::cin>>userName1;
	userName1 += "Donor.txt"; //adding extension of txt file so that it can read while logging in
	std::cout<<std::endl;
...
void Donor::loggedInOptions() { //this function have options after logIn 

		std::cout<<std::endl;
		std::cout<<"              Press [1] for Your Profile "<<std::endl;
		std::cout<<std::endl;
		std::cout<<"              Press [2] for Blood Donation "<<std::endl;
		std::cout<<std::endl;
...
void Donor::bloodDonated() { //fucntion for successful donations
	...
	std::cout<<"***   Thank you for Donating Blood     ***"<<std::endl<<std::endl;
	exit();
	std::cout<<"*** PROUND TO BE SOMEONE'S LIFELINE :) ***"<<std::endl;
}
void Donor::availbilityOfBlood() {

}
void Donor::listOfDonors() {
